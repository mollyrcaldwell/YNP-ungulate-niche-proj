...
spp_sample <- list()
for(i in 1:length(data_spp_sampled)){
  samp <- sample(unique(data_spp_sampled[[i]]$id_yr_seas), 10)
  
  spp_sample[[i]] <- data_spp_sampled[[i]] %>% filter(id_yr_seas %in% samp)
}
...
save.image("SSFs_state space_VariablesExtracted.RData")
