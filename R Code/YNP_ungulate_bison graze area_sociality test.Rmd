---
title: "YNP Ungulate sociality bison grazing areas"
...
#load bison grazing area shapefile (top 75% bison UDs)
bison_ga <- st_read("./Code output/dBBMM_UDs/UD_SUMS/dBBMM_UD_top75perc_ALLbison_growseas.shp")

#load all spp gps points
data <- readRDS("./Data/GPS data/Cleaned/allspp_cleanedGPSdata_seasonal_3.2022.rds")

#set bison grazing area to same crs as data
bison_ga <- st_transform(bison_ga, crs = st_crs(data))
```

#Split data to subsets of points within and outside bison grazing areas

```{r}
#create data subset with only GPS points that intersect with bison grazing areas
data_ga <- st_intersection(data, bison_ga)

#create subset with only points that do NOT intersect with bison grazing areas
data_no_ga <- data[!lengths(st_intersects(data, data_ga)), ]
```

#Grazing area network connections
#Prep data for network connections, spatsoc package

```{r}
#split geometry into x y columns
data_xy_ga <- data_ga %>%
    mutate(x = unlist(purrr::map(data_ga$geometry,1)),
           y = unlist(purrr::map(data_ga$geometry,2)))

data_xy_ga$geometry <- NULL

#convert data to data table for spatsoc
data_dt_ga <- setDT(data_xy_ga)

```

#Spatial network- by points (150 m, within 15 min)

##Temporal grouping

```{r}
#temporal group with threshold of x hours
g_data <- group_times(DT = data_dt_ga, datetime = 'date', threshold = "15 minutes")
```
...
group_pts(DT = g_data, threshold = 150, id = 'cid', 
