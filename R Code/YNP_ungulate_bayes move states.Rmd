...
data <- data %>%
  rename("id" = id_yr_seas, "SL" = dist, "TA" = rel.angle)
...
dist.bin.lims = quantile(bind_rows(data_list_filt[[1]])$dist,
...
```{r}
data_list <- data_list_filt[[1]]
data_list <- data_list[c(1:2)]

# Only retain id and discretized step length (SL) and turning angle (TA) columns
data_list2 <- purrr::map(data_list,
                   subset,
                   select = c("id", "SL", "TA"))
